 ...
 * @date 01.08.2007
 */
public class PingPong extends KineticLaw {

	public PingPong(PluginModel mod, int reactionnum, HashMap<String, String> idAndName2, List<String> modE, List<String> modActi, List<String> modInhib, boolean reversibility) 
	{
	...
	public void sMAK() 
	{
		...
		for(int i = 0; i < modE.size(); i++)
		{
			...
			paraList.add(kMr1);
				...
				nenner = kMr2 + "*" + specrefe1.getSpecies()
					+ "+" + kMr1 + "*" + specrefe2.getSpecies()
					+ "+" + specrefe1.getSpecies() + "*" + specrefe2.getSpecies();
				nennertex = kMr2tex + "\\cdot " + s.fromSpeciesToTex(specrefe1.getSpecies())
					+ "+" + kMr1tex + "\\cdot " + s.fromSpeciesToTex(specrefe2.getSpecies())
			...
			if(reaction.getReversible()==true||reversibility)
			{
				...
				zaehler = "(" + kcatp + "*"+ modE.get(i) + "*" + specrefe1.getSpecies()+ "*" + specrefe2.getSpecies() + ")/("
					...
					+ specrefp2.getSpecies() + ")/(" + kIp1 + "*" + kMp2 + ")";
				...
				zaehlertex = "\\frac{" + kcatptex + "\\cdot "+ s.fromSpeciesToTex(modE.get(i)) + "\\cdot " + s.fromSpeciesToTex(specrefe1.getSpecies())+ "\\cdot " + s.fromSpeciesToTex(specrefe2.getSpecies()) + "}{"
					...
					+ s.fromSpeciesToTex(specrefp2.getSpecies()) + "}{" + kIp1tex + "\\cdot " + kMp2tex + "}";
					
				nenner = "(" + specrefe1.getSpecies() + "/" + kIr1 + ")"
					+ " + (" + kMr1 + "*" + specrefe2.getSpecies() + "/" + kIr1 + "*" + kMr2 + ")"
					+ " + (" + specrefp1.getSpecies() + "/" + kIp1 + ")"
					+ " + (" + kMp1 + "*" + specrefp2.getSpecies() + "/" + kIp1 + "*" + kMp2 + ")"
					+ " + (" + specrefe1.getSpecies() + "*" + specrefe2.getSpecies() + "/" + kIr1 +"*" + kMr2 + ")"
					+ " + (" + specrefe1.getSpecies() + "*" + specrefp1.getSpecies() + "/" + kIr1 + "*" + kIp1+ ")"
					+ " + (" + kMr1 + "*" + specrefe2.getSpecies() + "*" + specrefp2.getSpecies() + "/" + kIr1 + "*" + kMr2 + "*" + kIp2 + ")"
					+ " + (" + specrefp1.getSpecies() + "*" + specrefp2.getSpecies() + "/" + kIp1 + "*" + kMp2 + ")"
					;
				nennertex = "\\frac{" + s.fromSpeciesToTex(specrefe1.getSpecies()) + "}{" + kIr1tex + "}"
					...
					+ " + \\frac{" + kMp1tex + "\\cdot " + s.fromSpeciesToTex(specrefp2.getSpecies()) + "}{" + kIp1tex + "\\cdot " + kMp2tex + "}"
					+ " + \\frac{" + s.fromSpeciesToTex(specrefe1.getSpecies()) + "\\cdot " + s.fromSpeciesToTex(specrefe2.getSpecies()) + "}{" + kIr1tex +"\\cdot " + kMr2tex + "}"
					+ " + \\frac{" + s.fromSpeciesToTex(specrefe1.getSpecies()) + "\\cdot " + s.fromSpeciesToTex(specrefp1.getSpecies()) + "}{" + kIr1tex + "\\cdot " + kIp1tex + "}"
					...
					+ " + \\frac{" + s.fromSpeciesToTex(specrefp1.getSpecies()) + "\\cdot " + s.fromSpeciesToTex(specrefp2.getSpecies()) + "}{" + kIp1tex + "\\cdot " + kMp2tex + "}"
					;
			}
