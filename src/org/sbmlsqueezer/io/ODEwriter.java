...
/**
 * This class writes the differential equations given by the
 * {@see KineticLawGenerator} to a LaTeX or plain text file.
 *
 * @since 2.0
 * @version
...
public class ODEwriter {
  ...
  /**
   * <p>This constructor analyzes the given file. If it ends with "*.txt",
   * a plain text file will be generated, which contains the ordinary
   * equation system. If the file ends with "*.tex", the system will
   * be written into a LaTeX file. Upper and lower cases for the file
   * ending are ignored. In all other cases, nothing will happen.</p>
   *
   * @param file
   * @throws IOException
   */
  public ODEwriter(File file, KineticLawGenerator klg) throws IOException {
    if ((new MyFileFilter(true, true)).accept(file)) {
      ...
      } else if (file.getPath().toLowerCase().toLowerCase().endsWith(".tex")) {
   ...
   * {@see KineticLawGenerator} into a plain text file.
  ...
  public final void writeTextFile(File file, KineticLawGenerator klg)
    ...
    for (i = 0; i < klg.getReactionNumAndKineticLaw().size(); i++) {
      String toWriteReaction = "Reaction: " + klg.getReactionNumAndId().get(i)
    ...
    for (i = 0; i < klg.getAllSpeciesNumAndIDs().size(); i++) {
      String toWrite = "Species: "
          ...
          + klg.getAllSpeciesNumAndIDs().get(i)
          + "]/dt = "
          + klg.getSpecieAndSimpleODE()
              .get(klg.getAllSpeciesNumAndIDs().get(i));
      append(toWrite, out);
      append(" ", out);
    }
    out.close();
  ...
  /**
   * This method writes the ordinary differential equation system given by the
   * {@see KineticLawGenerator} into a LaTeX file for further processing.
   * Note that the file extension does not matter.
   *
   * @param file
   * @param klg
  ...
  public final void writeLaTeXFile(File file, KineticLawGenerator klg)
    ...
    String title = "\\title{\\textsc{SBMLsqeezer}: Ordinary Differential Equation System ``";
    title += klg.getModel().getName().length() > 0 ? klg.getModel().getName()
        .replaceAll("_", " ") : klg.getModel().getId().replaceAll("_", " ");
    append(title + klg.getModel().getName() + "\"}", out);
    append(
   ...
   * @throws IOException
   */
  private final void append(String str, BufferedWriter writer) throws IOException {
    writer.write(str);
    writer.newLine();
  }
