...
The algorithm then iterates trough all reactions within the sub-model and performs several pre-processing steps, before an appropriate type of rate law can be selected:
\begin{enumerate}
...
In this step it is the aim to get hints if the reaction represents a transcription or translation:
  ...
  \begin{SCfigure}
    \includegraphics[width=.45\textwidth]{SBGN/gene-regulation}
...
After having reaction preprocessing and semantic checking completed, the algorithm assigns a list of the following thirteen general categories to each reaction within the sub-model:\\
\noindent
...
%\end{enumerate}
This list may either contain all applicable categories for the reaction or just the one category with highest priority.
...
\TODO{Wie wird die \URL genau zusammengebaut? Das ist genau der Schl\"ussel, um den es hier geht. Darum muss dazu unbedingt mehr geschrieben werden.}
