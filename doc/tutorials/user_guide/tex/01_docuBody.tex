...
SBMLsqueezer offers you to give a list of \KEGG \acp{ID} for those reactive
...
In the \CellDesigner plug-in version, just click on \menu{Plugin > SBMLsqueezer 2.0 > Squeeze kinetic laws} to launch the kinetics wizard.
...
The menu item \menu{Plugin > SBMLsqueezer 2.0} gives you access to nearly all functions of the program.
...
To ensure that entries for the reactions of interest can be found in \SABIO, it is important to annotate these reactions with \acp{ID} from the \KEGG database using the \MIRIAM registry.
...
When using SBMLsqueezer as a plug-in for \CellDesigner, you can either see the results by looking through the tables and overviews provided by \CellDesigner itself, or you can click on \menu{Plugin > SBMLsqueezer 2.0 > Show the data structure}. This will open a similar view as the stand-alone version provides.
...
In order to use the on-line version of SBMLsqueezer, open a browser of your choice.
...
\Cref{fig:WebserviceLaunch} displays the web interface of the Galaxy framework.
\begin{figure}
...
The \KEGG \ac{ID} of a reaction is automatically added to the search terms (compare \vref{sec:SABIOautomatic}).
...
\item[--ignore-these-species-when-creating-laws{[} |={]}<String>]
  ...
  \acp{ID} for several ions and small molecules, including water and
...
The user can define which kinds of species may be considered enzymes in the specific context.
...
\begin{enumerate}
  \item the algorithm creates a sub-model that only comprises those reactions for which rate laws are to be created.
All relevant model components, such as species, compartments, units etc. are copied into this sub-model.
Operating on this trimmed copy of the full model has the advantage that changes of the algorithm doe not affect the original data structure and can be easily disregarded.
All following steps are applied to each individual reaction in the sub-model.
  \item If the user defines a list of KEGG identifiers \citep{Kanehisa2000a} for species whose contribution to rate laws should be neglected, species with such terms in their \MIRIAM \citep{Le2005} annotation \citep{Courtot2011} are removed.
...
In this step the \SBO term \citep{Courtot2011} attribute of each reaction participant is analyzed in order to get hints if the reaction represents a transcription or translation.
%If a reactant represents a gene or gene-coding region, the reaction could be a transcription.
%If the reaction involves a reactant that stands for an RNA or mRNA molecule it could be a translation.
%If at least one modifier represents a gene or RNA molecule, the reaction could represent a translation.
  \item Based on their \SBO term \citep{Courtot2011} attribute all reaction modifiers are grouped into the following sets:
  \begin{itemize}
  \item enzymes,
  \item activators,
  \item inhibitors, and
...
In order to ensure that the algorithm can process each reaction, several cases for semantic rules are checked:
\begin{enumerate}
  \item If a reaction involves genes or gene-coding regions as reactants or its set of reactants is empty and all products are RNA molecules, the reaction is recognized as \emph{transcription}.
  \item If the substrate species of a reaction are RNA molecules or the reaction does not have any reactants and all products are forms of proteins, the reaction is recognized as \emph{translation}.
  \item If the stoichiometry of reactants and products is unity, the reaction can only be categorized as transcription if the only reactant is a gene or gene-coding region and it is only a valid translation if the only reactant is an RNA molecule.
Depending on user preferences, the algorithm can set the boundary condition for genes as part of this step.
Note that a set of reactants or products is said to be \emph{empty} if either no element has been assigned to this set, the stoichiometry of each element is zero, or if each element in the set is annotated with the \SBO term for empty set.
\end{enumerate}

After having model preprocessing and semantic checking completed, the algorithm assigns a list of the following thirteen general categories to each reaction within the sub-model:
\begin{enumerate}
...
\end{enumerate}
This list may either contain all possible categories or just the category with highest priority.
...
In case of unity stoichiometry on both sides of the reaction, and if the reaction follows the pattern of transcription or translation reactions, it is added to this category.
...
Next, the stoichiometry and directionality of the reaction are taken into account in order to determine if the reaction belongs to the uni-uni, bi-uni, or bi-bi reactions.
\end{enumerate}
The identification of the category with highest priority works very similar.
For reactions with empty list of reactants a gene-regulatory reaction has higher priority than the zeroth reactant order.
...
The group of those kinetic equations also implement a common interface and can therefore also be identified on run-time.
...
Finally, all relevant changes in the sub-model are synchronized to the original model.
...
If species have been deleted from reactions, this change is not synchronized.
