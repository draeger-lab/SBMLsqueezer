...
\vspace{\wrapfigspace}
\includegraphics[width=.8cm]{icons/jar_icon}
\end{wrapfigure}
...
should be replaced with newly generated rate equations, if a search in SABIO-RK
...
rate laws.
The other sections of this tab allow you to select which type ofrate law should
...
\begin{wrapfigure}{O}{.9cm}
\vspace{\wrapfigspace}
\includegraphics[width=16px]{icons/settings_64}
\end{wrapfigure}
...
under \MacOSX, these can be found under \menu{SBMLsqueezer > Preferences\dots}.
\MacOSX also gives you the option to access the preferences menu with the keystroke
...
\begin{wrapfigure}{O}{.9cm}
\vspace{\wrapfigspace}
\includegraphics[width=16px]{icons/folder_64}
\end{wrapfigure}
...
You can add a kinetic equation to each reaction separately (see figure \ref{fig:RateLawDialog} for details). This enables you to select the type of rate law that seems appropriate for you.
\begin{figure}[htbp]
\shadowimage[width=\textwidth]{Screenshot_2}
\caption{Generating a kinetic equation for a single reaction. This dialog can
...
The alternative to adding rate laws to each reaction separately is the generation of kinetic equations for all reactions in one step (for details see figure \ref{fig:KineticLawWizard}). 
\begin{figure}[htbp]
...
The tree on the left of the SBMLsqueezer window enables you to view the generated kinetic equations. You just have to expand "Reactions" and afterwards the desired reaction. Then you can click on "Kinetic Law" and the rate equation is displayed on the right in detail.
...
Starting from the screen in figure \ref{fig:KineticLawWizard} you have to click on \keys{Next}. On the next screen you see a table with the reactions and the suggested type of rate equation for each reaction (figure \ref{fig:suggestedEquations}). You can change a suggested rate law, if you are not satisfied with it (see figure \ref{fig:changeEquation}).

\begin{figure}[htbp]
\shadowimage[width=\textwidth]{Screenshot_5}
\caption{Table with suggested rate equation types.
This table is displayed, when you start from the screen in figure \ref{fig:KineticLawWizard} and click on \keys{Next}. The table shows which type of rate equation SBMLsqueezer suggests to generate for each reaction as well as a preview of the respective formula. You can add the rate laws to the model by clicking on \keys{Finish}.}
\label{fig:suggestedEquations}
\end{figure}

\begin{figure}[htbp]
\shadowimage[width=\textwidth]{Screenshot_6}
\caption{Choosing an alternative rate law.
If you are not satisfied with a rate law type suggested by SBMLsqueezer, you can click on the respective row in the table in figure \ref{fig:suggestedEquations}. Then this window is displayed which lets you choose another type of rate law.}
\label{fig:changeEquation}
\end{figure}

\subsection{For selected reactions}
You can generate a rate law for a specific reaction by right-clicking on it. Then a window appears, which lets you choose the type of rate law (see figure \ref{fig:RateLawDialog} for details).
...
\subsection{For an entire network}
Figures \ref{fig:startAutomatic} to \ref{fig:changes} show how you can add kinetic equations from SABIO-RK to an several reactions simultaneously. Please note: The reactions you want to equip with kinetic equations should be annotated with \KEGG reaction ids.
...
Press CTRL%\keys{\ctrl}
while clicking on these reactions and click on \keys{Next} when finished.}
...
You can remove a query field again by clicking on \keys{Delete}.}
...
rate equation has been found. If you are satisfied with the results, you can click on \keys{Next}.}
...
If you now click on \keys{Finish} and confirm the changes afterwards, the rate
...
As an alternative you can add kinetic equations from SABIO-RK to a selected reaction separately. This is shown in Figures \ref{fig:startManual} to \ref{fig:changesManual}.
...
You can remove a query field by clicking on \keys{Delete}
...
You can select a reaction and click on \keys{Next} or you can click on \keys{Back}
...
Now click on \keys{Finish} and confirm the changes afterwards. Then the rate equation will be
...
\file{SBMLsqueezer\_v2.0\_incl-libs.jar}. In this example, a minimum of 128\,MB
...
\end{lstlisting}
Under \Windows, you might use the command \file{javaw} instead of
