...
To abort this dialog, click at \keys{Cancel} or hit the \keys{\escwin} key.}
...
In this example two rows are highlighted in \colorbox{red}{red}.
...
The next window lists all reactions whose background color has the following meaning:
A reaction is marked in \colorbox{LimeGreen}{green} if a matching rate equation has been found,
\colorbox{Goldenrod}{yellow} if the found rate equations do not exactly fit to your model, and
\colorbox{red}{red} if no matching rate equation has been found. 
A found rate equation exactly fits to your model, if there is a straightforward matching of all elements (species, compartments, etc.) contained in the equation to respective \SBML elements in your model.
...
To this end, SBMLsqueezer provides a wizard that guides you through the selection of relevant reactions (\vref{fig:selectReations}) and provides several options to restrict the search in \SABIO (\vref{fig:searchTerms}).
...
The \KEGG identifier of a reaction is then always included in the respective search in order to avoid adding kinetic laws referring to other reactions.
You can further restrict your search by changing the bounds of pH, temperature, etc. in the right panel (see \vref{fig:searchTerms}).
...
Now click on \keys{Finish} and afterwards a dialog pops up asking you to confirm the changes. After this confirmation the rate equation will be
...
There you can either and scroll down until you find the tool SBMLsqueezer and click at the the program's logo, or you can click at the link in the list of tools on the left.
Alternatively, you can go directly to the service SBMLsqueezer\footnote{\url{http://webservices.cs.uni-tuebingen.de/tool_runner?tool_id=sbmlupload2}}.
...
You can start working with SBMLsqueezer by either clicking at the program's icon or the link ``Upload SBML file".}
\label{fig:WebserviceLaunch}
\end{figure}

The first step is now to upload your \SBML file of interest to the on-line program.
To this end, you can either click at the link ``Upload SBML file" on the left-hand side of the browser window, or click at the icon of SBMLsquezeer.
...
Second, click at the link ``Generate kinetic rate equations" on the left-hand side of the browser window.
...
The reason is because the Galaxy framework of the University of Tuebingen provides \SBMLLaTeX as a separate on-line program.
...
To this end, a new job to is scheduled that will be displayed in your history.
\begin{figure}
...
When the job is done, you can obtain the results by first clicking at the link ``Generated kinetics for\ldots", which will appear in the history column.
You can then click on the disk icon below in order to download the \SBML file that contains your results.
If you click at the eye icon, the central column will display a rendered version of your \SBML file, which is the view at all \XHTML notes within your file.
\begin{figure}
...
You can view the details of each file by clicking at the eye icon, or download the file by first clicking at the link in the headline of the respective entry (``Generated kinetics for\ldots") and then clicking at the disk icon.}
...
We will now discuss important aspects of the new \API of SBMLsqueezer and give use-case examples.
...
\lstinputlisting[caption={Assignment of a kinetic law to a reaction via the \acs{API} of SBMLsqueezer 2}, label={lst:SBMLsqueezer2API1}, firstline={47}, lastline={76}]{../../../examples/src/org/sbml/squeezer/test/SqueezeReaction.java}

\subsection{Querying \SABIO and merging the results into a local \SBML document}
The console mode of \SABIO is implemented in the class \texttt{ConsoleWizard} located in the package \texttt{org.sbml.squeezer.sabiork.wizard.console}. 
The following example shows how to start a query for the reactions in the model.
 The search can either comprise all reactions or just the reactions, to which a kinetic law has not been added yet. All search terms that can be applied to the model are listed at the beginning.
 The \KEGG identifier of a reaction is automatically added to the search terms (compare \vref{sec:SABIOautomatic}). A found kinetic law is added to some reaction, if all contained elements can be matched to \SBML elements in your model.
\lstinputlisting[caption={\SABIO query for the reactions of a model}, label={lst:SBMLsqueezer2API1}, firstline={96}, lastline={138}]{../../../test/org/sbml/squeezer/test/sabiork/SABIORKApplication.java}
...
Features that require an active internet connection are the on-line check for updates and following external links within on-line help.
\newline
...
Just contact the mailing list using the following e-mail address: \ding{41}
\href{mailto:sbmlsqueezer@googlegroups.com}{\texttt{sbmlsqueezer@google\-groups.com}}
